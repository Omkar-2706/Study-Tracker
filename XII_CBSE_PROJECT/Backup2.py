...
from data_store import *  # get_total_seconds, save_total, get_id

points_target = 0
total_tdy_seconds = get_total_seconds()
...
def get_log():
    ...
    print(ses_points)
    save_total(total_seconds, ses_points, points_target)
    subj = drop_down.get()
    print(subj)
    save_session(get_id(subj), subj, seconds, ses_points)
...
drop_down.pack()
print(drop_down.get())
sub_add = Button(subject_frame, text="Add Subject", command=lambda: raise_frames(add_sub_frame))
sub_add.pack()
...
new_sub.pack()
Button(add_sub_frame, text="Submit", command=lambda: (raise_frames(subject_frame), add_subject())).pack()
